  ...
  return (
    <div className="w-full">
      ...
      <div className="block md:hidden border border-dashed border-[#00E5CC] p-4 bg-[#0a0a0a]">
        <h2 className="text-[#00E5CC] font-mono mb-4 text-sm font-bold uppercase">
          Top Developer Hubs
        </h2>
        <div className="space-y-4">
          {sortedData.map((city, i) => (
            <div
              key={i}
              className="flex flex-col gap-1 text-xs font-mono border-b border-dashed border-[#1a1a1a] pb-2 last:border-0 last:pb-0"
            >
              ...
              <div className="flex justify-between text-white">
                <span className="text-gray-400">Top:</span> @{city.top_contributor} (
                {city.top_score.toLocaleString()})
              </div>
            </div>
          ))}
        </div>
