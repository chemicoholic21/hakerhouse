...
export function TrendingRepos({ initialRepos, initialTotal }: TrendingReposProps) {
  ...
  const fetchRepos = useCallback(
    async (offsetVal: number, language: string, sort: string, search: string, append: boolean) => {
      setIsLoading(true);
      try {
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-highlight">Repository Health</h2>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <Dropdown label="Sort" value={sortBy} options={sortOptions} onChange={handleSortChange} />
        <Dropdown
          label="Language"
            ...
            <div className="flex items-start gap-3 mb-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <span className="text-xs text-muted-foreground">
                  {repo.language || 'Unknown'} · {formatStars(repo.stars)} stars · Pushed{' '}
      ...
      <div ref={loadMoreRef} className="py-8 flex justify-center">
        ...
        {!hasMore && repos.length > 0 && !isLoading && (
          <span className="text-muted-foreground text-sm">All {total} repositories loaded</span>
        )}
      </div>
    </section>
  );
