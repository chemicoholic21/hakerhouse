...
const [{ count }] =
  await sql`SELECT COUNT(*)::int AS count FROM analyses WHERE top_repos_json IS NOT NULL`;
...
for (let off = 0; off < count; off += PAGE) {
  const rows =
    await sql`SELECT username, top_repos_json FROM analyses WHERE top_repos_json IS NOT NULL ORDER BY username LIMIT ${PAGE} OFFSET ${off}`;
  for (const row of rows) {
    ...
    for (const r of repos) {
      ...
      keySets.set(
        ...
        (keySets.get(Object.keys(r).sort().join(',')) || 0) + 1
      );
      const slug = resolve(r, row.username);
      if (slug && slug.includes('/')) resolvable++;
      else {
        unresolvable++;
        if (badExamples.length < 15) badExamples.push({ username: row.username, r });
      }
    }
  }
}
